    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3rem' }}>
                <h2 style={{ fontSize: '2rem', fontWeight: 800, letterSpacing: '-0.04em' }}>Customer Database</h2>
            </div>
            ...
            <div style={{ background: 'white', borderRadius: '24px', border: '1px solid var(--border)', overflow: 'hidden', boxShadow: 'var(--shadow)' }}>
                <table className="admin-table" style={{ margin: 0 }}>
                    <thead style={{ background: '#f8fafc' }}>
                        <tr>
                            <th style={{ padding: '1.25rem 1.5rem', color: 'var(--text-muted)' }}>Customer Name</th>
                            ...
                            <tr key={u._id} style={{ borderBottom: '1px solid var(--border)' }}>
                                <td style={{ padding: '1.25rem 1.5rem' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                        <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: u.is_staff ? 'var(--primary)' : 'var(--secondary)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 800, fontSize: '0.9rem' }}>
                                            {(u.first_name || 'U')[0]}{(u.last_name || 'U')[0]}
                                        </div>
                                        <div>
                                            <p style={{ fontWeight: 700, color: 'var(--text)' }}>{u.first_name} {u.last_name}</p>
                                            <p style={{ fontSize: '0.75rem', color: 'var(--secondary)' }}>@{u.username || 'user'}</p>
                                        </div>
                                    </div>
                                </td>
                                <td style={{ padding: '1.25rem 1.5rem' }}>
                                ...
                                <td style={{ padding: '1.25rem 1.5rem' }}>
                                    <span style={{ 
                                        ...
                                        fontSize: '0.75rem', 
                                        fontWeight: 700, 
                                        background: u.is_staff ? '#e0e7ff' : '#f1f5f9',
                                        color: u.is_staff ? '#4338ca' : '#475569',
                                        display: 'inline-flex',
                                        alignItems: 'center',
                                        gap: '0.35rem'
                                    }}>
                                        <Shield size={12} />
                                        {u.is_superadmin ? 'SUPERADMIN' : u.is_staff ? 'STAFF' : 'CUSTOMER'}
                                    </span>
                                </td>
                                <td style={{ padding: '1.25rem 1.5rem' }}>
