        ...
        <div className="container cart-container">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '3rem', alignItems: 'start' }}>
                <div className="cart-items">
                    {cart.items.map(item => (
                        <div key={item.product._id} className="cart-item" style={{ boxShadow: 'var(--shadow)' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginRight: '2rem', background: '#f1f5f9', padding: '0.5rem', borderRadius: '12px' }}>
                                <button onClick={() => updateQuantity(item.product._id, item.quantity - 1)} className="btn btn-sm" style={{ padding: '0.25rem', background: 'white', borderRadius: '8px', border: '1px solid var(--border)' }}><Minus size={14} /></button>
                                <span style={{ fontWeight: 700, minWidth: '20px', textAlign: 'center' }}>{item.quantity}</span>
                                <button onClick={() => updateQuantity(item.product._id, item.quantity + 1)} className="btn btn-sm" style={{ padding: '0.25rem', background: 'white', borderRadius: '8px', border: '1px solid var(--border)' }}><Plus size={14} /></button>
                            </div>
                            <button onClick={() => removeFromCart(item.product._id)} style={{ color: '#ef4444', border: 'none', background: '#fee2e2', padding: '0.75rem', borderRadius: '12px', cursor: 'pointer', transition: 'all 0.2s' }} aria-label="Remove item">
                                <Trash2 size={20} />
                            </button>
                        </div>
                    ))}
